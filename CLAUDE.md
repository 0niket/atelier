...
5. **Ignore Store Directory**: Never commit `store/` directory - it's for temporary session data only
...
│   └── spec-kit/     # Reference repos for inspiration
├── store/            # Session data (DO NOT COMMIT)
...
- Clone reference repositories here for analysis
- Prototype ideas before moving to `plugins/`
- Experiment freely - it's scratch space
- Currently contains: spec-kit (GitHub's Spec-Driven Development toolkit)

**`store/`** - Session data (NOT gitignored, but NEVER commit):
- Temporary session context and working files
- Generated by learn-run plugin hooks
- Treat as ephemeral - don't rely on persistence

**`docs/plans/`** - Design plans:
- Detailed planning documents for plugins
